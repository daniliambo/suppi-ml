...
class Logger:
    ...
    class Run:
        ...
        def __init__(self, name: str, storage: pd.DataFrame, path: str):
            self.name = name
        ...
        def save_logs(self):
            with open(self._path / f'{self.name}.json', 'w+') as f:
                try:
                    ...
                    self._storage.loc[self.name] = pd.Series(
                        [np.NAN] * self._storage.shape[1])
                    json.dump(self._storage.loc[self.name].to_dict(), f)
    ...
    def run(self, name: tp.Optional[str] = None):
        ...
        if name is None:
            ...
            raise NameError(
                "Run with given name already exists, name should be unique")
        else:
            name = name.replace(' ', '_')
        self._current_run = Logger.Run(
            name, self.leaderboard, self.path / name)
    ...
    @contextlib.contextmanager
    def clear(self, name: tp.Optional[str] = None):
        ...
        elif name not in self.leaderboard.index:
            raise NameError("Run with given name doesn't exist")
...
class ExperimentHandler:
    ...
    def score_test(self, estimator, metrics, run, test_data=None):
        ...
        run.log_values({key + '_test': value for key,
                       value in test_scores.items()})
    ...
    def score_cv(self, estimator, metrics, run):
        ...
        # compute crossval scores (easier done using corresponding sklearn method)
        scorers_dict = _check_multimetric_scoring(
            estimator=estimator, scoring=metrics)

        cross_val_results = sklearn.model_selection.cross_validate(
            estimator=estimator, X=self.X_train, y=self.y_train, scoring=scorers_dict, cv=self._cv_iterable, n_jobs=self._n_jobs)
    ...
    def generate_stacking_predictions(self, estimator, run):
        ...
        # choose the most informative method for stacking predictions
        ...
        # generate crossval predictions for stacking using most informative method

        cross_val_stacking_prediction = sklearn.model_selection.cross_val_predict(
            estimator=estimator, X=self.X_train, y=self.y_train, cv=self._cv_iterable, n_jobs=self._n_jobs, method=method)
    ...
    def run(self, estimator: sklearn.base.BaseEstimator, name=None):
        ...
        with self.logger.run(name=name) as run:
            ...
            fitted_on_train, _, _ = self.generate_stacking_predictions(
                estimator, run)
            self.score_test(fitted_on_train, metrics,
                            run, test_data=self.X_test)
            return self.logger.leaderboard.loc[[run.name]]

    def clear(self, name=None):
        self.logger.clear(name)
    ...
    def get_stacking_predictions(self, run_names):
        ...
        for run_name in run_names:
            train_filename = self.logger.path / run_name / \
                ExperimentHandler.stacking_prediction_filename
            train_dataframes.append(load_predictions_dataframe(filename=train_filename, column_prefix=run_name,
                                                               index=self.X_train.index))
            test_filename = self.logger.path / run_name / \
                ExperimentHandler.test_stacking_prediction_filename
...
score_cv = ExperimentHandler.score_cv
generate_stacking_predictions = ExperimentHandler.generate_stacking_predictions
